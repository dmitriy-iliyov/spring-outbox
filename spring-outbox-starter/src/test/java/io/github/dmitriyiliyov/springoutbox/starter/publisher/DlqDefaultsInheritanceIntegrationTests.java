...
public class DlqDefaultsInheritanceIntegrationTests {
    ...
    private ApplicationContextRunner getDlqPropertiesRunner() {
        return getBaseContextRunner().withPropertyValues(
                ...
                "outbox.publisher.dlq.batch-size=500",
                "outbox.publisher.dlq.polling.type=adaptive",
                "outbox.publisher.dlq.polling.initial-delay=5m",
                "outbox.publisher.dlq.polling.min-fixed-delay=1s",
                "outbox.publisher.dlq.polling.max-fixed-delay=2m",
                "outbox.publisher.dlq.polling.multiplier=10.0"
        );
    }
    ...
    void partialOverride_transferTo_inheritsRestOfPolling() {
        getDlqPropertiesRunner()
                .withPropertyValues(
                        "outbox.publisher.dlq.transfer-to.polling.multiplier=2.5"
                )
                .run(context -> {
                    assertThat(context).hasNotFailed();
    ...
    void completeOverride_transferFrom_switchesToFixedPolling() {
        getDlqPropertiesRunner()
                .withPropertyValues(
                        "outbox.publisher.dlq.transfer-from.batch-size=1000",
                        "outbox.publisher.dlq.transfer-from.polling.type=fixed",
                        "outbox.publisher.dlq.transfer-from.polling.initial-delay=5m",
                        "outbox.publisher.dlq.transfer-from.polling.fixed-delay=1m"
                )
                .run(context -> {
                    assertThat(context).hasNotFailed();
    ...
    void disabledDlq_respectsFlag() {
        getBaseContextRunner()
                .withPropertyValues(
                        "outbox.publisher.dlq.enabled=false"
                )
                .run(context -> {
                    assertThat(context).hasNotFailed();
                    OutboxProperties properties = context.getBean(OutboxProperties.class);
    ...
    void fixedPolling_fallbackTo5sDefault() {
        getDlqPropertiesRunner()
                .withPropertyValues(
                        "outbox.publisher.dlq.transfer-to.polling.type=fixed"
                )
                .run(context -> {
                    assertThat(context).hasNotFailed();
