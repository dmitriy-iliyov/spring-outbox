...
import io.github.dmitriyiliyov.springoutbox.web.it.BaseOracleIntegrationTests;
...
@Transactional
class OracleOutboxDlqWebRepositoryIntegrationTests extends BaseOracleIntegrationTests {

    private final OutboxDlqWebRepository repository;
    private final MultiDialectOutboxDlqWebRepositoryIntegrationTests delegate;

    public OracleOutboxDlqWebRepositoryIntegrationTests(
            @Qualifier("oracleOutboxDlqWebRepository") OutboxDlqWebRepository repository,
    ...
    @Test
    @DisplayName("IT findBatchByStatus() Oracle pagination returns correct page sizes")
    void findBatchByStatus_oraclePagination_correctPageSizes() {
        delegate.saveBatch(
                IntStream.range(0, 7)
        ...
        List<OutboxDlqEvent> page1 = repository.findBatchByStatus(DlqStatus.MOVED, 0, 3);
        List<OutboxDlqEvent> page2 = repository.findBatchByStatus(DlqStatus.MOVED, 1, 3);
        List<OutboxDlqEvent> page3 = repository.findBatchByStatus(DlqStatus.MOVED, 2, 3);
    ...
    @Test
    @DisplayName("IT findBatchByStatus() Oracle pagination pages do not overlap")
    void findBatchByStatus_oraclePagination_noDuplicatesAcrossPages() {
        delegate.saveBatch(
                IntStream.range(0, 6)
                        .mapToObj(i -> delegate.buildEvent(DlqStatus.MOVED))
                        .toList()
